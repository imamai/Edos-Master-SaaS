...
import { Search, Plus, Package, Edit2, Trash2, ArrowUpCircle, History, Truck } from 'lucide-react'
    ...
    <div className="space-y-5">
      ...
      <div className="flex gap-1 bg-slate-100 dark:bg-slate-800 rounded-xl p-1 w-fit">
        {([
          ...
          { id: 'movements' as const, label: 'Stock Movements', icon: History },
        ]).map((t) => (
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
...
/* ──────────────────────────────────────────────────────────────
   STOCK MOVEMENTS TAB — trail of every stock change: goods received
   from a supplier (GRN), sales, manual adjustments, damage, etc.
────────────────────────────────────────────────────────────── */
type MovementType = 'purchase' | 'sale' | 'return' | 'adjustment' | 'transfer' | 'damage'
...
const MOVEMENT_LABELS: Record<MovementType, string> = {
  purchase: 'Goods Received', sale: 'Sale', return: 'Return',
  adjustment: 'Adjustment', transfer: 'Transfer', damage: 'Damage',
}
const MOVEMENT_COLORS: Record<MovementType, string> = {
  purchase: 'bg-green-100 dark:bg-green-950 text-green-700 dark:text-green-400',
  sale: 'bg-blue-100 dark:bg-blue-950 text-blue-700 dark:text-blue-400',
  return: 'bg-purple-100 dark:bg-purple-950 text-purple-700 dark:text-purple-400',
  adjustment: 'bg-amber-100 dark:bg-amber-950 text-amber-700 dark:text-amber-400',
  transfer: 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300',
  damage: 'bg-red-100 dark:bg-red-950 text-red-600 dark:text-red-400',
}
...
function StockMovementsTab() {
  ...
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')
  const [historyProduct, setHistoryProduct] = useState<MovementProduct | null>(null)
  ...
  useEffect(() => {
    ...
    supabase.from('product_stock')
      ...
      .then(({ data }) => { setProducts((data as MovementProduct[]) ?? []); setLoading(false) })
  }, [tenantId])
  ...
  return (
    <div className="space-y-4">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800">
                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">Product</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">SKU</th>
                <th className="text-right px-4 py-3 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">Current Stock</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {loading ? (
                Array.from({ length: 6 }).map((_, i) => (
                  <tr key={i}><td colSpan={3} className="px-4 py-3"><div className="h-4 bg-slate-100 dark:bg-slate-800 rounded animate-pulse" /></td></tr>
                ))
              ) : filtered.length === 0 ? (
                <tr><td colSpan={3} className="text-center py-16 text-slate-400 dark:text-slate-500">
                ...
                filtered.map((p) => (
                  <tr key={p.id} onClick={() => setHistoryProduct(p)}
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <History className="w-4 h-4 text-slate-300 dark:text-slate-600 flex-shrink-0" />
                    ...
                    <td className="px-4 py-3 text-right font-semibold text-slate-800 dark:text-white">{p.stock_quantity} {p.unit}</td>
                  </tr>
                ))
              )}
            </tbody>
      ...
      {historyProduct && (
        <ProductMovementModal product={historyProduct} onClose={() => setHistoryProduct(null)} />
      )}
    </div>
  )
}

function ProductMovementModal({ product, onClose }: { product: MovementProduct; onClose: () => void }) {
  const supabase = createClient()
  const [movements, setMovements] = useState<StockMovement[]>([])
  const [grnRefs, setGrnRefs] = useState<Record<string, GRNRef>>({})
  const [loading, setLoading] = useState(true)
  ...
  useEffect(() => {
    supabase.from('stock_movements')
      ...
      .then(async ({ data }) => {
        ...
        // reference_id is polymorphic (GRN, sale, ...) so it can't be
        // embedded directly — batch-fetch the GRNs it points at to show
        // the supplier + invoice number behind each "Goods Received" row.
        const grnIds = Array.from(new Set(
          rows.filter((m) => m.reference_type === 'grn' && m.reference_id).map((m) => m.reference_id as string)
        ))
        if (grnIds.length > 0) {
          const { data: grns } = await supabase.from('goods_received_notes')
            .select('id, grn_number, received_date, supplier_invoice_ref, supplier:suppliers(name)')
            .in('id', grnIds)
          const map: Record<string, GRNRef> = {}
          for (const g of (grns as (GRNRef & { id: string })[]) ?? []) map[g.id] = g
          setGrnRefs(map)
        }
        setLoading(false)
      })
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-slate-800 flex-shrink-0">
          ...
          <button onClick={onClose} className="p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 dark:text-slate-500">✕</button>
        </div>
          ...
          <table className="w-full text-sm">
            <thead className="sticky top-0 bg-white dark:bg-slate-900">
              <tr className="border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800">
                ...
                <th className="text-center px-4 py-3 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">Type</th>
                ...
                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">Supplier / Reference</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {loading ? (
                Array.from({ length: 4 }).map((_, i) => (
                  <tr key={i}><td colSpan={5} className="px-4 py-3"><div className="h-4 bg-slate-100 dark:bg-slate-800 rounded animate-pulse" /></td></tr>
                ))
              ) : movements.length === 0 ? (
                ...
                movements.map((m) => {
                  const grn = m.reference_type === 'grn' && m.reference_id ? grnRefs[m.reference_id] : undefined
                  return (
                    <tr key={m.id}>
                      ...
                      <td className="px-4 py-3 text-center">
                        <span className={`text-xs px-2 py-1 rounded-full font-medium ${MOVEMENT_COLORS[m.type] ?? MOVEMENT_COLORS.adjustment}`}>
                          {MOVEMENT_LABELS[m.type] ?? m.type}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-right">
                        <span className={`font-bold ${m.quantity > 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                          {m.quantity > 0 ? '+' : ''}{m.quantity} {product.unit}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-slate-600 dark:text-slate-300 text-xs">{m.branch?.name ?? '—'}</td>
                      <td className="px-4 py-3 text-xs">
                        {grn ? (
                          <div className="flex items-center gap-1.5">
                            <Truck className="w-3.5 h-3.5 text-slate-400 dark:text-slate-500 flex-shrink-0" />
                            <div>
                              <p className="text-slate-700 dark:text-slate-200">{grn.supplier?.name ?? 'Unknown supplier'}</p>
                              <p className="text-slate-400 dark:text-slate-500 font-mono">
                                {grn.grn_number}{grn.supplier_invoice_ref ? ` · Inv: ${grn.supplier_invoice_ref}` : ''}
                              </p>
                            </div>
                          </div>
                        ) : (
                          <span className="text-slate-500 dark:text-slate-400">{m.notes || m.reference_type || '—'}</span>
                        )}
                      </td>
                    </tr>
                  )
